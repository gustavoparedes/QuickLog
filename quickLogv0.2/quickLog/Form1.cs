...
namespace quickLog
{
    public partial class FormMain : Form
    {
        ...
        // Definir un único StreamWriter para mantener el archivo de log abierto durante toda la ejecución
        ...
        public FormMain(string filePath) : this()
        {
            ...
            if (!string.IsNullOrEmpty(filePath))
            {
                ...
                this.Text = "Quick Log v 0.2 is working on:   " + dbname;
    ...
    !labelColorDictionary.ContainsKey(row.Cells["Label"].Value.ToString()))
        ...
        private async Task WriteToLogFile(string message)
        {
            try
            {
                ...
                {
                    Console.WriteLine("Logger is not initialized.");
                }
        ...
        ////Priemra funcion de escribir log, requiere un sleep que no es la solucion
                        ...
                        try
                        {
                            using (EventLogReader logReader = new EventLogReader(query))
                            {
                                ...
                                while ((entry = logReader.ReadEvent()) != null)
                                {
                                    ...
                                    //Prueba de guardar fecha / hora en utc, no funciono
                                    //DateTime time = (entry.TimeCreated ?? DateTime.MinValue).ToUniversalTime();

                                    // Obtén el tiempo desde TimeCreated
                                    DateTime time = entry.TimeCreated ?? DateTime.MinValue;

                                    // Extrae el tiempo UTC directamente desde el XML si existe
                                    string messageXML = entry.ToXml();
                                    DateTime xmlTime = ExtractTimeFromXml(messageXML);

                                    // Utiliza xmlTime si está presente, de lo contrario usa time
                                    DateTime finalTime = xmlTime != DateTime.MinValue ? xmlTime : time;
                                    ...
                                    {
                                        command.Parameters.AddWithValue("@TimeCreated", finalTime);
        ...
        private DateTime ExtractTimeFromXml(string xml)
        {
            try
            {
                // Crea un documento XML para analizar messageXML
                var xmlDoc = new System.Xml.XmlDocument();
                xmlDoc.LoadXml(xml);

                // Intenta extraer el nodo con la marca de tiempo
                var timeNode = xmlDoc.SelectSingleNode("//TimeCreated/@SystemTime");

                if (timeNode != null && DateTime.TryParse(timeNode.Value, out DateTime extractedTime))
                {
            ...
            {
                // Manejo de errores si hay problemas al analizar el XML
            }

            // Retorna un valor mínimo si no se pudo extraer el tiempo
        ...
        private async void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            using (OpenFileDialog OpenFileDialog = new OpenFileDialog())
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    //logFilePath = $"{dbname}.log";
                    try
                    {
                        if (logWriter == null) // Inicializa solo si aún no está inicializado
                        {
                    ...
                    {
                        MessageBox.Show("Error initializing log: The file is being used by another application: " + Ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Application.Exit();
                    }
