...
namespace quickLog
{
    public partial class FormMain : Form
    {
        ...
        // Definir un único StreamWriter y seguro para mantener el archivo de log abierto durante toda la ejecución
        ...
        public FormMain()
        {
            ...
            // Registramos la funcion de sqlite para regexp
        ...
        public FormMain(string filePath) : this()
        {
            ...
            if (!string.IsNullOrEmpty(filePath))
            {
                ...
                this.Text = "QuickLog v0.3 is working on:   " + dbname;
        ...
        public class MyRegEx : SQLiteFunction
        {
            public override object Invoke(object[] args)
            {
                ...
                // args[0]: el patrón de regex, args[1]: el texto a evaluar
        ...
        private void DataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            //AL cambiar el modo de sleccion de celda a fila se cambio a esto:
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                {
                    return; // Salir del evento y no realizar ninguna operación
                }
                ...
                // Recorrer cada celda de la fila seleccionada
                foreach (DataGridViewCell cell in selectedRow.Cells)
                {
                    // Obtener el encabezado de la columna y el valor de la celda
                    string columnName = cell.OwningColumn.HeaderText;
                    string cellValue = cell.Value?.ToString() ?? "N/A";

                    // Aplicar estilo al título (columna)
                    ...
                    // Aplicar estilo al contenido (valor de la celda)
                    //Si la Columna es TimeCreated se agrega el utcoffset
                    richTextBox_Detail.SelectionFont = new Font(richTextBox_Detail.Font, FontStyle.Regular);
                    richTextBox_Detail.SelectionColor = Color.Black;
                    if (columnName == "TimeCreated")
                    {
                        if (utcOffset > 0)
                        {
                            richTextBox_Detail.AppendText($"{cellValue}  Timezone (UTC): +{utcOffset.ToString()}\n");
                        }
                        else
                        {
                            richTextBox_Detail.AppendText($"{cellValue}  Timezone (UTC): {utcOffset.ToString()}\n");
                    ...
                    // Línea en blanco después de cada par título/campo
    ...
    !labelColorDictionary.ContainsKey(row.Cells["Label"].Value.ToString()))
        ...
        //Funciona perfectamente sin regexp
        ...
        // Hasta aqui el coemntario de  //Funciona perfectamente sin regexp
        ...
        private void highlight_Text()
        {
            if (searchTermExists)
            {
                ...
                // Si el término es una expresión regular
            ...
            if (search2TermExists)
            {
                ...
                {
                    HighlightPlainText(text1, Color.Yellow);
                    HighlightPlainText(text2, Color.LightPink);
                }

                //// Resalta el segundo término
                //if (checkBox_regexp.Checked)
                //{
                //    HighlightWithRegex(text2, Color.LightPink);
                //}
        ...
        // Método para resaltar texto simple
        private void HighlightPlainText(string text, Color highlightColor)
        {
            ...
            while (startIndex < richTextBox_Detail.TextLength)
            {
                ...
                if (wordStartIndex != -1)
                {
                    richTextBox_Detail.SelectionStart = wordStartIndex;
                    richTextBox_Detail.SelectionLength = text.Length;
                    richTextBox_Detail.SelectionBackColor = highlightColor;
                    richTextBox_Detail.Refresh();
        ...
        // Método para resaltar texto usando expresiones regulares
        private void HighlightWithRegex(string pattern, Color highlightColor)
        {
            try
            {
                ...
                {
                    richTextBox_Detail.SelectionStart = match.Index;
                    richTextBox_Detail.SelectionLength = match.Length;
                    richTextBox_Detail.SelectionBackColor = highlightColor;
                    richTextBox_Detail.Refresh();
                }
            }
            ...
            {
                MessageBox.Show($"Error en la expresión regular: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        //Ajusta el query sql para el offset del tiempo



        private string AdjustDatetimeQuery(string baseQuery)
        {
            // Construye la parte del ajuste del offset
            string offsetString = (utcOffset >= 0 ? "+" : "") + utcOffset + " hours";

            // Lista explícita de columnas con ajuste a TimeCreated
            ...
            // Si TimeCreated está explícito en la consulta, ajústalo directamente
            if (baseQuery.Contains("TimeCreated"))
            {
                return baseQuery.Replace("TimeCreated", $"STRFTIME('%Y-%m-%d %H:%M:%f', TimeCreated, '{offsetString}')");
            }

            // Retorna la consulta original si no hay nada que ajustar
            return baseQuery;
        }
        ...
        // Inicio primer botón / menú View All Logs
        private void ShowAllLogs()
        {
            string query = "SELECT * from LogData;";

            query = AdjustDatetimeQuery(query);
        ...
        // Inicio segundo submenú General interest
        ...
        private void UserWasEnabled()
        {
            ...
            query = AdjustDatetimeQuery(query); // Aplica el ajuste a TimeCreated
        ...
        public void InitializeLog(string logFilePath)
        {
            lock (logLock)
            {
                try
                ...
                {
                    MessageBox.Show("Log bloqueado por otro proceso");
                    Application.Exit();
                }
            }
        }
        ...
        //        //    logWriter = new StreamWriter(logFilePath, append: true); // true para añadir al log si ya existe
        //        //}

        //    }
        //    catch (Exception ex)
        //    {
        //        LogToConsole($"Error writing to log: {ex.Message}");
        //    }
        //}
                        ...
                        try
                        {
                            using (EventLogReader logReader = new EventLogReader(query))
                            {
                                ...
                                while ((entry = logReader.ReadEvent()) != null)
                                {
                                    ...
                                    DateTime exactUtcTime = AdjustToExactUtc(time);
                                    ...
                                    {
                                        command.Parameters.AddWithValue("@TimeCreated", exactUtcTime);
        ...
        private void Windows_Defender_Menu()
        {
            ...
            save_query_count("Deleted_History_Malware", Deleted_History_Malware);
        ...
        private void newToolStripMenuItem_Click(object sender, EventArgs e)
        {
            using (SaveFileDialog saveFileDialog = new SaveFileDialog())
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    string dbNameWithoutExtension = Path.GetFileNameWithoutExtension(dbname);
                    logFilePath = Path.Combine(Path.GetDirectoryName(dbname), $"{dbNameWithoutExtension}.log");
                    //Cerrar el log si es que hay alguno abierto
        ...
        private void enable_buttons()
        {
            ...
            label_about.Enabled = true;
            labelUTC.Enabled = true;
...

        {
            ...
            label_about.Enabled = false;
            labelUTC.Enabled = false;
        ...
        private void Btn_Seacrh_Click(object sender, EventArgs e)
        {
            ...
            bool firstSearchArgument = false;
            ...
            if (ConnectionString != "")
            {
                ...
                //Capturamos el rango de fechas y sa,imos si esta mal, mayor la fecha de inicio que la de fin
                ...
                string sd = AdjustToUtc(StartDate, utcOffset);
                string ed = AdjustToUtc(EndDate, utcOffset);
                string offsetString = (utcOffset >= 0 ? "+" : "") + utcOffset + " hours";

                string sql_base_query = $@"SELECT Log_id, STRFTIME('%Y-%m-%d %H:%M:%f', TimeCreated, '{offsetString}') AS TimeCreated, 
                UserID, EventID, MachineName, Level, LogName, EventMessage, EventMessageXml, ActivityID, Label, Comment FROM LogData 
                WHERE";
                string sql_query_time = "";
                
                if (checkBox_time.Checked) {
                    sql_query_time = $" (TimeCreated >= '{sd}' AND TimeCreated <= '{ed}')";
                    firstSearchArgument = true;
                    //LogToConsole("Fist argument a True en Time");
                }
                ...
                if (firstSearchArgument)
                {
                    ...
                    if (SelectedItems.Count > 0)
                    {
                        if (SelectedItems.Count == 1)
                        {
                            sql_query_UserID = $" (UserID is \"{SelectedItems[0]}\")";
                        }
                        else
                        {
                            for (int i = 0; i < SelectedItems.Count; i++)
                        ...
                        firstSearchArgument = true;
                        //LogToConsole("Fist argument a True en UserID");
                    }                   
                }
                ...
                if (firstSearchArgument)
                {
                    ...
                    if (SelectedItems.Count > 0)
                    {
                        if (SelectedItems.Count == 1)
                        {
                            sql_query_EventID = $" (EventID is \"{SelectedItems[0]}\")";
                        }
                        else
                        {
                            for (int i = 0; i < SelectedItems.Count; i++)
                        ...
                        firstSearchArgument = true;
                        //LogToConsole("Fist argument a True en EventID");
                    }                   
                }
                ...
                if (firstSearchArgument)
                {
                    ...
                    if (SelectedItems.Count > 0)
                    {
                        if (SelectedItems.Count == 1)
                        {
                            sql_query_MachineName = $" (MachineName is \"{SelectedItems[0]}\")";
                        }
                        else
                        {
                            for (int i = 0; i < SelectedItems.Count; i++)
                        ...
                        firstSearchArgument = true;
                        //LogToConsole("Fist argument a True en Machinename");
                    }                    
                }
                ...
                if (firstSearchArgument)
                {
                    ...
                    if (SelectedItems.Count > 0)
                    {
                        if (SelectedItems.Count == 1)
                        {
                            sql_query_Level = $" (Level is \"{SelectedItems[0]}\")";
                        }
                        else
                        {
                            for (int i = 0; i < SelectedItems.Count; i++)
                        ...
                        firstSearchArgument = true;
                        //LogToConsole("Fist argument a True en level");
                    }                    
                }
                ...
                if (firstSearchArgument)
                {
                    ...
                    if (SelectedItems.Count > 0)
                    {
                        if (SelectedItems.Count == 1)
                        {
                            sql_query_LogName = $" (LogName is \"{SelectedItems[0]}\")";
                        }
                        else
                        {
                            for (int i = 0; i < SelectedItems.Count; i++)
                        ...
                        firstSearchArgument = true;
                        //LogToConsole("Fist argument a True en Logname");
                    }
                }
                ...
                if (firstSearchArgument)
                {
                    ...
                    if (SelectedItems.Count > 0)
                    {
                        if (SelectedItems.Count == 1)
                        {
                            sql_query_LabelName = $" (Label is \"{SelectedItems[0]}\")";
                        }
                        else
                        {
                            for (int i = 0; i < SelectedItems.Count; i++)
                        ...
                        firstSearchArgument = true;
                        //LogToConsole("Fist argument a True en Label");
                    }
                }
                ...
                //La version de abajo corrige el paso de REGEXP al sqlite pero no resalta los dos terminos, solo el primero

                string sql_query_EventMessage = "";

                if (!string.IsNullOrEmpty(textBox_SearchTerm.Text))
                {
                    string searchInput = textBox_SearchTerm.Text.Trim();

                    // Validar si contiene comillas dobles (carácter prohibido en este caso)
                    ...
                    // Dividir el término de búsqueda en partes basadas en operadores AND y OR
                    ...
                    if (firstSearchArgument == true)
                    {
                        if (countAnd == 0 && countOr == 0)
                        {
                            ...
                            if (useRegex)
                            {
                                sql_query_EventMessage += $" and (EventMessage REGEXP \"{searchTerm}\" OR EventMessageXML REGEXP \"{searchTerm}\")";
                            }
                            else
                            {
                        ...
                        {
                            // Caso de términos múltiples con un solo operador (AND o OR)
                            search2TermExists = true;
                            string[] parts = countAnd > 0
                                ? Regex.Split(searchInput, @"\band\b", RegexOptions.IgnoreCase)
                                : Regex.Split(searchInput, @"\bor\b", RegexOptions.IgnoreCase);

                            if (parts.Length == 2)
                            {
                                string firstTerm = searchTerm = parts[0].Trim();
                                string secondTerm = searchTerm2 = parts[1].Trim();

                                if (useRegex)
                                {
                                    sql_query_EventMessage += countAnd > 0
                                        ? $" and ((EventMessage REGEXP \"{firstTerm}\" AND EventMessage REGEXP \"{secondTerm}\") OR (EventMessageXML REGEXP \"{firstTerm}\" AND EventMessageXML REGEXP \"{secondTerm}\"))"
                                        : $" and ((EventMessage REGEXP \"{firstTerm}\" OR EventMessage REGEXP \"{secondTerm}\") OR (EventMessageXML REGEXP \"{firstTerm}\" OR EventMessageXML REGEXP \"{secondTerm}\"))";
                                }
                                else
                                {
                                    sql_query_EventMessage += countAnd > 0
                                        ? $" and ((EventMessage LIKE \"%{firstTerm}%\" AND EventMessage LIKE \"%{secondTerm}%\") OR (EventMessageXML LIKE \"%{firstTerm}%\" AND EventMessageXML LIKE \"%{secondTerm}%\"))"
                                        : $" and ((EventMessage LIKE \"%{firstTerm}%\" OR EventMessage LIKE \"%{secondTerm}%\") OR (EventMessageXML LIKE \"%{firstTerm}%\" OR EventMessageXML LIKE \"%{secondTerm}%\"))";
                                }
                            }
                        }
                        else
                        {
                            MessageBox.Show("Only one AND or OR, please", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            return;
                        }
                    }
                    else {
                            ...
                            if (countAnd == 0 && countOr == 0)
                            {
                                ...
                                if (useRegex)
                                {
                                    sql_query_EventMessage += $" (EventMessage REGEXP \"{searchTerm}\" OR EventMessageXML REGEXP \"{searchTerm}\")";
                                }
                                else
                                {
                                    sql_query_EventMessage += $" (EventMessage LIKE \"%{searchTerm}%\" OR EventMessageXML LIKE \"%{searchTerm}%\")";
                                }
                            }
                            else if ((countAnd == 1 && countOr == 0) || (countAnd == 0 && countOr == 1))
                            {
                                ...
                                if (parts.Length == 2)
                                {
                                    ...
                                    {
                                        sql_query_EventMessage += countAnd > 0
                                            ? $" ((EventMessage REGEXP \"{firstTerm}\" AND EventMessage REGEXP \"{secondTerm}\") OR (EventMessageXML REGEXP \"{firstTerm}\" AND EventMessageXML REGEXP \"{secondTerm}\"))"
                                            : $" ((EventMessage REGEXP \"{firstTerm}\" OR EventMessage REGEXP \"{secondTerm}\") OR (EventMessageXML REGEXP \"{firstTerm}\" OR EventMessageXML REGEXP \"{secondTerm}\"))";
                                    }
                                    else
                                    {
                                        sql_query_EventMessage += countAnd > 0
                                            ? $" ((EventMessage LIKE \"%{firstTerm}%\" AND EventMessage LIKE \"%{secondTerm}%\") OR (EventMessageXML LIKE \"%{firstTerm}%\" AND EventMessageXML LIKE \"%{secondTerm}%\"))"
                                            : $" ((EventMessage LIKE \"%{firstTerm}%\" OR EventMessage LIKE \"%{secondTerm}%\") OR (EventMessageXML LIKE \"%{firstTerm}%\" OR EventMessageXML LIKE \"%{secondTerm}%\"))";
                                    }
                                }
                            }
                            else
                ...
                //Hasta aqui conetario de version que corrige REGEXP


                string Full_Sql_Query = "";
                
                Full_Sql_Query = sql_base_query + sql_query_time + sql_query_UserID + sql_query_EventID + sql_query_MachineName + sql_query_Level + sql_query_LogName + sql_query_LabelName
                + sql_query_EventMessage;

                if (Full_Sql_Query == sql_base_query)
                {
                    Full_Sql_Query = $@"SELECT Log_id, STRFTIME('%Y-%m-%d %H:%M:%f', TimeCreated, '{offsetString}') AS TimeCreated, 
                UserID, EventID, MachineName, Level, LogName, EventMessage, EventMessageXml, ActivityID, Label, Comment FROM LogData";
                }
                ...
                if (checkBox_showSql.Checked) 
                {
                    LogToConsole(Full_Sql_Query);
                }
                    
                //LogToConsole($"checkBox_time.Checked: {checkBox_time.Checked}");
                //LogToConsole($"UserID CheckedItems Count: {sfComboBox_UserID.CheckedItems.Count}");
                //LogToConsole($"EventID CheckedItems Count: {sfComboBox_EventID.CheckedItems.Count}");
                //LogToConsole($"MachineName CheckedItems Count: {sfComboBox_MachineName.CheckedItems.Count}");
                //LogToConsole($"Level CheckedItems Count: {sfComboBox_Level.CheckedItems.Count}");
                //LogToConsole($"LogName CheckedItems Count: {sfComboBox_LogName.CheckedItems.Count}");
                //LogToConsole($"Label CheckedItems Count: {sfComboBox_Label.CheckedItems.Count}");
                //LogToConsole($"First Search Argument: {firstSearchArgument}");
        ...
        private void btn_Make_Time_Range_Filter_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows.Count == 2 && hasColumn == true)
            {
                ...
                if (start < end)
                {
                    ...
                    query = AdjustDatetimeQuery(query);
                    ShowQueryOnDataGridView(query);
                    if (checkBox_showSql.Checked) LogToConsole(query);
        ...
        private async void btn_savetopdf_Click(object sender, EventArgs e)
        {
            ...
            // Verificar si existe la columna "Label" en el DataGridView
            foreach (DataGridViewColumn columna in dataGridView1.Columns)
            {
                if (columna.Name == "Label")
                {
            ...
            if (hasColumn)
            {
                ...
                if (saveFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        LogToConsole("Please wait, generating PDF. This process may take some time depending on the number of logs to generate.");
                        groupBox_Main.Enabled = false;
                        groupBox_CustomSearch.Enabled = false;

                        // Usar Task.Run para ejecutar la lógica pesada en un hilo en segundo plano
        ...
        private void GeneratePdf(string filename)
        {
            ...
            using (FileStream fileStream = new FileStream(filename, FileMode.Create))
            {
                ...
                // Asignar la clase FooterHandler para manejar el pie de página
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (!row.IsNewRow)
                    {
                        // Recorrer todas las columnas dinámicamente
                        foreach (DataGridViewCell cell in row.Cells)
                        {
                            // Agregar el título de la subsección (encabezado de columna)
                            ...
                            string cellValue = cell.Value?.ToString() ?? "N/A";
                            Paragraph content = new Paragraph(cellValue, FontFactory.GetFont(FontFactory.HELVETICA, 10));
                            document.Add(content);
                        }

                        // Iniciar una nueva página después de cada fila
                        document.NewPage();
                    }
                }
    ...
    Terminal_Server_Session_Disconnected = Terminal_Server_Session_Reconnection = Terminal_Server_Session_Disconnected_Code = Terminal_Server_Session_Disconnected_by_Session = Terminal_Server = 0;
        ...
        {
            utcOffset = (int)numericUpDown1.Value;
            //LogToConsole($"Using utc offset {utcOffset}");
        }
